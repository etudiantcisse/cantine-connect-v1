...
const PAYMENT_OPTIONS = [
  { id: "wave", label: "Wave", caption: "Paiement instantané", color: "#2563EB" },
  { id: "orange_money", label: "Orange Money", caption: "Paiement sécurisé", color: "#EA580C" },
  { id: "cash", label: "Espèces", caption: "Paiement en caisse", color: "#059669" },
];
  ...
  return (
    <View style={styles.itemRow}>
      ...
      <View style={styles.itemRight}>
        <Text style={styles.itemPrice}>{formatFcfa(item.product.prix * item.quantity)}</Text>
        <View style={styles.itemControls}>
          ...
          <Pressable style={[styles.ctrlBtn, styles.ctrlBtnDanger]} onPress={onRemove}>
...
export default function CartScreen({ navigation }) {
  const { user } = useAuth();
  const { items, groupedByVendor, subtotal, clearCart, removeItem, setItemQuantity } = useCart();
  ...
  const handleCheckout = async () => {
    if (!user?.id) { Alert.alert("Panier", "Vous devez être connecté."); return; }
    if (groupedByVendor.length === 0) { Alert.alert("Panier", "Votre panier est vide."); return; }
    ...
    try {
      ...
      Alert.alert("Commande", error.message ?? "Erreur de validation");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <View style={styles.emptyContainer}>
        <View style={styles.emptyIconWrap}>
          <MaterialCommunityIcons name="cart-outline" size={52} color={colors.primary} />
        </View>
        <Text style={styles.emptyTitle}>Panier vide</Text>
        <Text style={styles.emptyText}>Ajoutez des produits avant de commander.</Text>
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          {SLOTS.map((slot) => (
            <Pressable
              ...
              style={[
                ...
                slot.disabled && selectedSlot !== slot.id && styles.slotDisabled,
              ]}
            ...
            >
              ...
              <Text style={[styles.slotLabel, selectedSlot === slot.id && { color: "#059669" }]}>
                {slot.label}
              </Text>
            </Pressable>
          ))}
        ...
        <View style={styles.paymentWrap}>
          {PAYMENT_OPTIONS.map((opt) => (
            <Pressable
              key={opt.id}
              style={[styles.paymentCard, modePaiement === opt.id && { borderColor: opt.color, borderWidth: 2 }]}
              onPress={() => setModePaiement(opt.id)}
            >
              <View style={[styles.paymentIconWrap, { backgroundColor: opt.color + "18" }]}>
                <Text style={[styles.paymentIconText, { color: opt.color }]}>
                  {opt.id === "wave" ? "W" : opt.id === "orange_money" ? "OM" : "€"}
                </Text>
              </View>
              <View style={{ flex: 1 }}>
              ...
              <View style={[styles.radio, modePaiement === opt.id && { backgroundColor: opt.color, borderColor: opt.color }]} />
            </Pressable>
          ))}
        </View>
      </ScrollView>
      ...
      <View style={styles.footer}>
        ...
        >
          ...
          <MaterialCommunityIcons name="lock-outline" size={16} color="rgba(255,255,255,0.8)" />
        </Pressable>
        <Text style={styles.secureNote}>PAIEMENT 100% SÉCURISÉ PAR CANTINE CONNECTÉE</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  emptyTitle: { color: "#1e293b", fontFamily: "Manrope_800ExtraBold", fontSize: 22, marginBottom: 6 },
  emptyText: { color: "#94A3B8", fontFamily: "Manrope_500Medium", fontSize: 14, textAlign: "center" },
  emptyBtn: {
    marginTop: 20,
  ...
  headerTitle: { color: "#1e293b", fontFamily: "Manrope_800ExtraBold", fontSize: 18 },
  ...
  itemImgWrap: { width: 68, height: 68, borderRadius: 14, overflow: "hidden", backgroundColor: "#F1F5F9" },
  itemImg: { width: "100%", height: "100%" },
  itemImgFallback: { flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#FFF7ED" },
  itemName: { color: "#1e293b", fontFamily: "Manrope_700Bold", fontSize: 15 },
  itemQty: { color: "#94A3B8", fontFamily: "Manrope_500Medium", fontSize: 12, marginTop: 2 },
  itemRight: { alignItems: "flex-end", gap: 6 },
  itemPrice: { color: "#059669", fontFamily: "Manrope_800ExtraBold", fontSize: 15 },
  ...
  ctrlBtnText: { color: "#334155", fontFamily: "Manrope_700Bold", fontSize: 16 },
  ...
  slotLabel: { color: "#94A3B8", fontFamily: "Manrope_500Medium", fontSize: 10, marginTop: 3 },
  paymentWrap: { gap: 10 },
  paymentCard: {
    flexDirection: "row",
    alignItems: "center",
  ...
  paymentLabel: { color: "#1e293b", fontFamily: "Manrope_700Bold", fontSize: 15 },
  paymentCaption: { color: "#94A3B8", fontFamily: "Manrope_500Medium", fontSize: 12, marginTop: 1 },
  radio: {
    width: 22,
    height: 22,
    borderRadius: 11,
  ...
  totalLabel: { color: "#94A3B8", fontFamily: "Manrope_500Medium", fontSize: 13 },
  totalAmount: { color: "#1e293b", fontFamily: "Manrope_800ExtraBold", fontSize: 30 },
  slotBadge: { backgroundColor: "#DCFCE7", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 99 },
  slotBadgeText: { color: "#059669", fontFamily: "Manrope_700Bold", fontSize: 12 },
  confirmBtn: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  confirmText: { color: "white", fontFamily: "Manrope_800ExtraBold", fontSize: 17 },
  secureNote: {
    textAlign: "center",
    color: "#CBD5E1",
